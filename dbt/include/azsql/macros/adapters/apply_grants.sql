{% macro azsql__get_show_grant_sql(relation) %}
    select
        GRANTEE as grantee,
...
{%- macro azsql__get_grant_sql(relation, privilege, grantees) -%}
    {%- set grantees_safe = [] -%}
...
{%- macro azsql__get_revoke_sql(relation, privilege, grantees) -%}
    {%- set grantees_safe = [] -%}
    {%- for grantee in grantees -%}
        {%- set grantee_safe = "[" ~ grantee ~ "]" -%}
        {%- do grantees_safe.append(grantee_safe) -%}
...
{% macro azsql__apply_grants(relation, grant_config, should_revoke=True) %}
